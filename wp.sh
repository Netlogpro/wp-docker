...
WP_MULTISITE_TYPE="${WP_MULTISITE_TYPE:-subdirectory}"
...
write_caddyfile() {
  mkdir -p config
  # Subdomain multisite needs the apex + wildcard host so site1.domain resolves.
  local caddy_hosts="${SITE_HOST}"
  if multisite_enabled && [ "$(multisite_type)" = "subdomain" ]; then
    caddy_hosts="${SITE_HOST}, *.${SITE_HOST}"
  fi
...
# Note: Let's Encrypt wildcard certs (*.domain) need a DNS challenge provider.
# Apex-only HTTPS works without DNS plugins; add DNS config for wildcards.
...
multisite_type() {
  case "${WP_MULTISITE_TYPE,,}" in
    subdomain|subdomains) printf '%s' "subdomain" ;;
    *) printf '%s' "subdirectory" ;;
  esac
}

validate_multisite() {
  multisite_enabled || return 0

  local type
  type="$(multisite_type)"
  case "${WP_MULTISITE_TYPE,,}" in
    subdirectory|subdir|subdomain|subdomains) ;;
    *)
      echo "Error: WP_MULTISITE_TYPE must be 'subdirectory' or 'subdomain' (got: ${WP_MULTISITE_TYPE})." >&2
      exit 1
      ;;
  esac
  ...
  if [ "$type" = "subdomain" ]; then
    local host
    host="$(extract_site_host "${WP_SITE_URL_RESOLVED}")"
    if [ -z "${WP_SITE_URL:-}" ] || [ "$host" = "localhost" ] || [ "$host" = "127.0.0.1" ]; then
      echo "Error: subdomain multisite requires WP_SITE_URL with a real hostname (not localhost)." >&2
      echo "Example: WP_SITE_URL=newsite.local" >&2
      exit 1
    fi
  fi
}
...
is_wp_multisite() {
  local flag
  flag="$($DC exec -T wordpress wp --allow-root eval 'echo is_multisite() ? 1 : 0;' 2>/dev/null | tr -d '[:space:]')"
  [ "$flag" = "1" ]
}

# Fresh network install, or convert an existing single site. Idempotent when
# Multisite is already enabled.
ensure_multisite() {
  multisite_enabled || return 0

  local type domain path
  type="$(multisite_type)"
  domain="$(extract_site_host "$WP_SITE_URL_RESOLVED")"
  path="/"

  if is_wp_multisite; then
    echo "==> Multisite already enabled (${type}); syncing network domain constants..."
    $DC exec -T wordpress wp --allow-root config set MULTISITE true --raw --type=constant \
      || echo "    Warning: could not set MULTISITE."
    if [ "$type" = "subdomain" ]; then
      $DC exec -T wordpress wp --allow-root config set SUBDOMAIN_INSTALL true --raw --type=constant \
        || echo "    Warning: could not set SUBDOMAIN_INSTALL."
    else
      $DC exec -T wordpress wp --allow-root config set SUBDOMAIN_INSTALL false --raw --type=constant \
        || echo "    Warning: could not set SUBDOMAIN_INSTALL."
    fi
    $DC exec -T wordpress wp --allow-root config set DOMAIN_CURRENT_SITE "$domain" --type=constant \
      || echo "    Warning: could not set DOMAIN_CURRENT_SITE."
    $DC exec -T wordpress wp --allow-root config set PATH_CURRENT_SITE "$path" --type=constant \
      || echo "    Warning: could not set PATH_CURRENT_SITE."
    return 0
  fi

  if [ "$type" = "subdomain" ]; then
    echo "==> Converting / enabling Multisite (subdomain)..."
    $DC exec -T wordpress wp --allow-root core multisite-convert \
      --title="${WP_SITE_TITLE}" \
      --subdomains \
      || { echo "Error: multisite-convert (subdomain) failed." >&2; exit 1; }
  else
    echo "==> Converting / enabling Multisite (subdirectory)..."
    $DC exec -T wordpress wp --allow-root core multisite-convert \
      --title="${WP_SITE_TITLE}" \
      || { echo "Error: multisite-convert (subdirectory) failed." >&2; exit 1; }
  fi
}
...
if multisite_enabled; then
  echo "==> Multisite: enabled ($(multisite_type))"
fi
...
if ! $DC exec -T wordpress wp --allow-root core is-installed >/dev/null 2>&1; then
  if multisite_enabled; then
    echo "==> Installing WordPress Multisite ($(multisite_type))..."
    if [ "$(multisite_type)" = "subdomain" ]; then
      $DC exec -T wordpress wp --allow-root core multisite-install \
...
# Convert existing single-site → Multisite when enabled; sync network constants.
ensure_multisite
if multisite_enabled; then
  $DC exec -T wordpress wp --allow-root rewrite flush --hard
fi
...
$(if multisite_enabled; then
  ...
  echo "   Network:   ${WP_SITE_URL_RESOLVED}/wp-admin/network/"
  if [ "$(multisite_type)" = "subdirectory" ]; then
    echo "   Sites:     ${WP_SITE_URL_RESOLVED}/site-slug/"
  else
    echo "   Sites:     <scheme>://site-slug.$(extract_site_host "$WP_SITE_URL_RESOLVED")/"
  fi
fi)
...
$(if [ -n "${WP_SITE_URL:-}" ] && [[ "$WP_SITE_URL_RESOLVED" != *"localhost"* ]]; then
  ...
  if multisite_enabled && [ "$(multisite_type)" = "subdomain" ]; then
    echo "                127.0.0.1  site1.$(extract_site_host "$WP_SITE_URL_RESOLVED")  # each subsite (no *.wildcards on Windows)"
  fi
fi)
$(if [ "$SSL_MODE" = "local" ]; then
  echo ""
...
$(if [ "$SSL_MODE" = "global" ]; then
  ...
  if multisite_enabled && [ "$(multisite_type)" = "subdomain" ]; then
    echo "   Wildcard HTTPS (*.${SITE_HOST}) needs a DNS challenge provider in Caddy."
  fi
fi)
