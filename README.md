...
├── docker-compose.ssl.yml       # Caddy HTTPS override (local-ssl / global-ssl)
├── Dockerfile                   # Extends official WordPress image (WP-CLI, Composer)
├── .env.example                 # Environment template (copy → .env)
├── config/
│   ├── bootstrap.php            # Loads wp-config snippets + HTTPS proxy trust
│   ├── Caddyfile.local.example  # Reference shape for local-ssl
│   ├── Caddyfile.global.example # Reference shape for global-ssl
│   ├── Caddyfile                # Generated by wp.sh when SSL is enabled
│   ├── php-uploads.ini          # Generated by wp.sh from PHP_* .env values
...
./wp.sh local-ssl        # Start with HTTPS (Caddy local CA — .local domains)
./wp.sh global-ssl       # Start with HTTPS (Let's Encrypt — public DNS)
...
`local-ssl` and `global-ssl` are **mutually exclusive**. Pass exactly one when starting the stack. They can be combined with phpMyAdmin:
...
Both modes put **Caddy** in front of WordPress as a reverse proxy, force `https://` for `siteurl` / `home`, and teach WordPress to trust `X-Forwarded-Proto` (via `config/bootstrap.php`).

| Mode | Cert | Needs | Typical domain |
|---|---|---|---|
| `local-ssl` | Caddy internal CA (in-container) + auto host trust | `WP_SITE_URL` | `newsite.local` |
| `global-ssl` | Let's Encrypt | `WP_SITE_URL` + real `WP_ADMIN_EMAIL` + public DNS on 80/443 | `example.com` |

**Local SSL example**

TLS terminates **inside the Caddy container**. After Caddy starts, `wp.sh` exports that CA and trusts it on the host so the browser can show a Secure padlock:

- **WSL2** → imports into the Windows Current User *Trusted Root* store (click **Yes** if Windows shows a Security Warning)
- **macOS** → adds to the login keychain
- **Linux** → installs into the system CA store when passwordless `sudo` is available

```bash
# .env
WP_SITE_URL=newsite.local

# /etc/hosts (and on WSL2, the Windows hosts file too)
127.0.0.1 newsite.local

./wp.sh local-ssl
# → https://newsite.local/

# Re-run only the export + trust step (stack already running):
./wp.sh trust-ssl
```

Fully quit and reopen the browser after a successful trust. If the Windows dialog was cancelled, run `./wp.sh trust-ssl` again and click **Yes**.

> For a publicly trusted cert with no host trust step, use a real domain with `global-ssl` (Let's Encrypt).
...
WP_ADMIN_EMAIL=you@example.com   # used for Let's Encrypt registration

./wp.sh global-ssl
# → https://example.com/
```

DNS for the hostname must point at this machine, and ports `HTTP_PORT` (default 80) and `HTTPS_PORT` (default 443) must be reachable from the internet. `WP_ADMIN_EMAIL` must be a real address — the default `admin@example.com` is rejected by Let's Encrypt.

Starting **without** an SSL flag clears a previous SSL setup (`--remove-orphans` removes Caddy). `down` / `sync` / `reset` remember the last SSL mode via `.ssl-mode`.
...
|---|---|---|
| `WORDPRESS_PORT` | `8080` | Host port mapped to WordPress (ignored for publishing when SSL is on) |
| `PHPMYADMIN_PORT` | `8081` | Host port for phpMyAdmin — must differ from `WORDPRESS_PORT` / SSL ports |
| `HTTP_PORT` | `80` | Caddy HTTP port (redirect + ACME) when using SSL |
| `HTTPS_PORT` | `443` | Caddy HTTPS port when using SSL |
...
### PHP upload / request limits

Written by `wp.sh` to `config/php-uploads.ini` and mounted into the WordPress container. Applied on every `./wp.sh` / `./wp.sh sync` (Apache is reloaded). `PHP_POST_MAX_SIZE` defaults to `PHP_UPLOAD_MAX_FILESIZE` when left blank. phpMyAdmin’s `UPLOAD_LIMIT` follows `PHP_UPLOAD_MAX_FILESIZE`.

| Variable | Default | Description |
|---|---|---|
| `PHP_UPLOAD_MAX_FILESIZE` | `64M` | Max size of an uploaded file |
| `PHP_POST_MAX_SIZE` | `64M` | Max size of POST body (should be ≥ upload max) |
| `PHP_MAX_FILE_UPLOADS` | `20` | Max number of files per request |
| `PHP_MAX_EXECUTION_TIME` | `300` | Max seconds a script may run |
| `PHP_MAX_INPUT_TIME` | `300` | Max seconds spent parsing input |
| `PHP_MEMORY_LIMIT` | *(empty)* | Optional PHP `memory_limit` (separate from `WP_MEMORY_LIMIT`) |
